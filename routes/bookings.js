...
const { sendBookingConfirmationToClient, sendBookingNotificationToPhotographer } = require('../services/emailService');
...
router.post('/', protect, [
  ...
  try {
    ...
    try {
      const clientName = populatedBooking.client?.name || 'Client';
      const clientEmail = populatedBooking.client?.email;
      const photographerName =
        populatedBooking.photographer?.user?.name || 'Photographer';
      ...
      const photographerEmail = populatedBooking.photographer?.user?.email;
      if (photographerEmail) {
        sendBookingNotificationToPhotographer(
          ...
          .then(() =>
            console.log(`📧 Booking notification sent to photographer ${photographerEmail}`)
          )
          .catch((err) =>
            console.error('Failed to send photographer notification:', err.message)
          );
      }
    } catch (emailErr) {
      // Never block the booking response due to email failures
