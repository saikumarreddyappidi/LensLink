...
router.put('/profile', protect, authorize('photographer'), [
  ...
  try {
    ...
    const updateFields = {};
    const allowedFields = ['bio', 'specialties', 'location', 'pricing', 'experience', 'socialMedia', 'profilePhoto', 'backupCount'];
...
router.post('/portfolio', protect, authorize('photographer'), [
  ...
  body('imageUrl')
    ...
    .custom((value) => {
      // Accept both base64 data URLs and regular http(s) URLs
      if (!value) return false;
      if (value.startsWith('data:image/')) return true;
      if (value.startsWith('http://') || value.startsWith('https://')) return true;
      throw new Error('Please provide a valid image URL or base64 data URL');
    }),
  body('category')
    .optional()
